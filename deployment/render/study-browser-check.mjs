...
try{
 ...
 await evaluate('document.querySelector("input[name=answer][value=1]").click()');await click('submit');
 ...
 await evaluate('document.querySelector("input[name=answer][value=0]").click()');await click('submit');await wait('document.getElementById("save-status").textContent.includes("Progress saved")','correct saved answer');
 ...
 await click('logout');await wait('!document.getElementById("login").hidden','logout');
